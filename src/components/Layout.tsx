...
const navItems: NavItem[] = [
  ...
  { path: '/student/notices', label: '通知中心', icon: <Bell size={20} />, roles: ['student'] },
  ...
  { path: '/admin/students', label: '学生管理', icon: <GraduationCap size={20} />, roles: ['admin'] },
  { path: '/admin/exams', label: '试卷管理', icon: <ClipboardList size={20} />, roles: ['admin'] },
  { path: '/admin/questions', label: '题库管理', icon: <BookOpen size={20} />, roles: ['admin'] },
  { path: '/admin/config', label: '测评配置', icon: <Settings size={20} />, roles: ['admin'] },
  { path: '/admin/templates', label: '模板配置', icon: <FileText size={20} />, roles: ['admin'] },
  { path: '/admin/courses', label: '课程管理', icon: <CalendarDays size={20} />, roles: ['admin'] },
  { path: '/admin/ai-config', label: 'AI配置', icon: <Cpu size={20} />, roles: ['admin'] },
];
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <aside className="hidden lg:flex fixed left-0 top-0 h-full w-64 bg-white border-r border-slate-200 flex-col z-50">
        <div className="p-6 flex items-center gap-3">
          ...
          <div>
            <h1 className="font-bold text-lg text-slate-800">智能测评系统</h1>
            <p className="text-xs text-slate-500">素质教育机构</p>
          </div>
        </div>
        ...
        <div className="p-4 border-t border-slate-200">
          ...
          <button
            ...
            className="w-full flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-medium text-slate-600 hover:bg-red-50 hover:text-red-600 transition-all duration-200 mt-2"
          >
            <LogOut size={18} />
            退出登录
          </button>
