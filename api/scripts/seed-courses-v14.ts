...
export async function seedCoursesV14() {
  const db = await getDb();

  // Migrate courses table: add new columns if not exist
  try {
    const courseCols = await db.all("PRAGMA table_info(courses)");
    const colNames = courseCols.map((c: any) => c.name);
  ...
  // Migrate courses table: update course_type constraint
  try {
  ...
  // Migrate questions table: update course_type constraint and data
  try {
    ...
    if (qTable && qTable.sql && !qTable.sql.includes("'scratch'")) {
      ...
      const hasUsageCount = backupCols.some((c: any) => c.name === 'usage_count');
  ...
  // Migrate exams table: update course_type data
  try {
